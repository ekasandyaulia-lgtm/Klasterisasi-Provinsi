...
# Halaman Dinamika Temporal
    ...
    if not selected_provinsi:
        ...
        with col_radar:
            st.subheader("Profil Kekuatan Indikator (Radar Chart)")
            ...
            st.markdown(
                f"<small><i>Skala 0-100 menunjukkan kekuatan relatif tiap indikator pada tahun {tahun_radar}. Semakin lebar area, semakin dominan.</i></small>",
                unsafe_allow_html=True
            )

            # Filter data yang sudah dinormalisasi (0-100)
            ...
            if not df_radar_filtered.empty:
                ...
                # Labels untuk sudut radar
                theta_labels = [ind_labels[col] for col in indikator_cols]
                theta_labels.append(theta_labels[0]) 

                for prov in selected_provinsi:
                    prov_data = df_radar_filtered[df_radar_filtered['Provinsi'] == prov]
                    if not prov_data.empty:
                        r_values = prov_data[indikator_cols].iloc[0].tolist()
