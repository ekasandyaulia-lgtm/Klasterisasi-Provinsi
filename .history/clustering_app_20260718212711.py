...
with col_menu:
    halaman = option_menu(
        ...
        styles={
            ...
            "nav": {"justify-content": "flex-end", "align-items": "center"},
            "nav-item": {"flex-grow": "0", "flex-basis": "auto"},
...
# Halaman Dinamika Temporal
    ...
    if not selected_provinsi:
        ...
        # Heatmap Chart
        # Ganti blok Heatmap/Radar Chart sebelumnya dengan Faceted Bar Chart
        with col_radar: 
            st.subheader("Perbandingan Nilai Aktual Indikator")
            
            tahun_bar = st.selectbox(
                "Pilih Tahun Analisis Kinerja:", 
                options=sorted(df['Tahun'].unique(), reverse=True),
                key="tahun_bar_profil"
            )
            
            st.markdown(
                f"<small><i>Menampilkan perbandingan nilai transaksi aktual antar provinsi terpilih pada tahun {tahun_bar}. Skala sumbu Y dibebaskan dan menyesuaikan otomatis untuk tiap indikator.</i></small>",
                unsafe_allow_html=True
            )

            # Menggunakan dataframe asli (df), bukan yang dinormalisasi (df_norm)
            df_bar_filtered = df[
            ...
            if not df_bar_filtered.empty:
                # Transformasi data (melt) agar formatnya memanjang (long format) untuk Plotly
                ...
                # Ubah nama indikator agar lebih enak dibaca menggunakan dictionary ind_labels
                df_melt['Indikator'] = df_melt['Indikator'].map(ind_labels)

                # Buat Grouped/Faceted Bar Chart
                fig_bar = px.bar(
                    df_melt, 
                    x='Provinsi', 
                    y='Nilai', 
                    color='Provinsi', 
                    facet_col='Indikator',
                    facet_col_wrap=2, # Membentuk grid 2x2
                    text_auto='.2s' # Menampilkan angka di pucuk bar (dibulatkan)
                )

                # KUNCI UTAMA: Lepaskan kuncian sumbu Y agar tiap kotak punya skalanya sendiri
                ...
                fig_bar.update_layout(
                    ...
                    showlegend=False # Tidak butuh legenda karena sumbu X sudah nama provinsi
                )
                
                # Bersihkan embel-embel teks bawaan Plotly (misal: "Indikator=Outflow Tunai")
                fig_bar.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1], font=dict(weight="bold")))
                fig_bar.for_each_xaxis(lambda x: x.update(title='')) # Sembunyikan tulisan 'Provinsi' di sumbu X
    ...
    st.subheader("Ringkasan Alur Pemrosesan Data & Pemodelan")
    st.markdown(
    ...
    st.subheader("Konfigurasi Parameter Hiperparameter Model")
    ...
    # Matrik Eval
    st.subheader("Validitas Internal & Metrik Evaluasi")
    st.markdown(
    ...
    # PCA 
    st.divider()
    st.subheader("Visualisasi Ruang Sebaran Komponen Utama (PCA)")
