...
# Halaman Dinamika Temporal
    ...
    # Baris atas Sankey Diagram (lebar penuh)
    # Menampilkan alur perpindahan klaster tiap provinsi dari tahun ke tahun
    st.subheader("Alur Perpindahan Klaster Provinsi (Sankey Diagram)")
    try:
        with open("indonesia.geojson", "r") as f:
            geojson_temporal = json.load(f)

        # Membuat data untuk Sankey Diagram
        sankey_data = []
        ...
        labels = list(set(sankey_counts['source']).union(set(sankey_counts['target'])))
        label_indices = {label: idx for idx, label in enumerate(labels)}

        sankey_counts['source_idx'] = sankey_counts['source'].map(label_indices)
        sankey_counts['target_idx'] = sankey_counts['target'].map(label_indices)

        fig_sankey = px.sankey(
            sankey_counts,
            node=dict(label=labels),
            link=dict(source=sankey_counts['source_idx'], target=sankey_counts['target_idx'], value=sankey_counts['count']),
            title="Perpindahan Klaster Provinsi dari Tahun ke Tahun"
        )
