...
# Halaman Dinamika Temporal
    ...
    try:
        ...
        # Label unik untuk node (full, dipakai untuk indexing & x_position)
        ...
        # Hindari x=0 dan x=1 persis (Plotly Sankey kadang error di posisi ekstrem)
        x_positions = [min(max(x, 0.001), 0.999) for x in x_positions]

        # Label pendek untuk node — hanya nama klaster, tahun dipindah ke anotasi
        label_klaster_pendek = [label.split(" - ", 1)[1] for label in unique_labels]

        # Palet warna per klaster — sesuaikan key dengan label asli di Target_Semantic
        palet_warna = {
            "Maju (Digital Hub)": "#2ECC71",
            "Berkembang (Transitioning)": "#F1C40F",
            "Tertinggal (Cash-Reliant)": "#E74C3C",
        }
        warna_default = "#95A5A6"  # fallback jika ada label baru yang tak terdaftar

        node_colors = [
            palet_warna.get(klaster, warna_default) for klaster in label_klaster_pendek
        ]
        ...
        # Anotasi tahun di atas tiap kolom (menggantikan label tahun yang dibuang dari node)
        ...
        st.caption(
            "Warna node: 🟢 Maju (Digital Hub) · 🟡 Berkembang (Transitioning) · 🔴 Tertinggal (Cash-Reliant). "
