...
if halaman == "Overview":
    ...
    with col_btn:
        st.button("Unduh PDF", width='stretch')
    st.divider()
    ...
    with c1:
            ...
            try:
                ...
                fig_map = px.choropleth(
                    ...
                    color_discrete_sequence=[palet_warna.get(klaster, warna_default) for klaster in df_map_overview['Target_Semantic'].unique()],
                    hover_name='Provinsi',
                    projection="mercator"
                )
        ...
        try:
            ...
            if mode_tampilan == "Peta Statis":
                fig = px.choropleth(
                    ...
                    featureidkey="properties.name",
                    color='Target_Semantic',
                    color_discrete_sequence=[palet_warna.get(klaster, warna_default) for klaster in df_map['Target_Semantic'].unique()],
                    hover_name='Provinsi',
...
# Halaman Dinamika Temporal
    ...
    with col_tren:
        ...
        if avail_cols:
            ...
            fig_trend = px.line(df_trend_melted, x='Tahun', y='Nilai Rata-rata', color='Indikator', 
                                markers=True, color_discrete_sequence=[palet_warna.get(klaster, warna_default) for klaster in df_trend_melted['Indikator'].unique()])
            ...
            for idx, col_name in enumerate(indikator_cols):
                with (grid_c1 if idx % 2 == 0 else grid_c2):
                    fig_small = px.line(
                        ...
                        markers=True,
                        color_discrete_sequence=[palet_warna.get(klaster, warna_default) for klaster in df_line_filtered['Target_Semantic'].unique()]
                    )
    ...
    metrics_data = {
        ...
        'WCSS (Inersia)': [18.2045, 25.5276, 18.2128, 19.7207, 22.8123],
        'Silhouette Score': [0.4587, 0.4109, 0.4854, 0.4924, 0.4497],
        'Calinski-Harabasz Score': [63.3552, 45.5801, 68.6326, 65.3940, 56.4187],
        'Davies-Bouldin Score': [0.5076, 0.5614, 0.4783, 0.4727, 0.5446]
    }
    ...
    def buat_grafik_metrik(df, y_col, judul, penjelasan, ambang, warna):
        fig = px.line(df, x='Tahun', y=y_col, markers=True, color_discrete_sequence=[warna])
        fig.update_layout(
    ...
    with m1:
        st.plotly_chart(buat_grafik_metrik(df_metrics, 'WCSS (Inersia)', 'WCSS / Inertia', 'Mengukur kepadatan internal.', 'Ambang: Lebih Rendah = Lebih Rapat', '#E74C3C'), use_container_width=True)
    with m2:
        st.plotly_chart(buat_grafik_metrik(df_metrics, 'Silhouette Score', 'Silhouette Score', 'Mengukur derajat pemisahan.', 'Ambang: Mendekati 1 = Terpisah Baik', '#2ECC71'), use_container_width=True)
    with m3:
        st.plotly_chart(buat_grafik_metrik(df_metrics, 'Calinski-Harabasz Score', 'Calinski-Harabasz', 'Rasio dispersi antar-dalam klaster.', 'Ambang: Lebih Tinggi = Lebih Baik', '#3498DB'), use_container_width=True)
    with m4:
        st.plotly_chart(buat_grafik_metrik(df_metrics, 'Davies-Bouldin Score', 'Davies-Bouldin', 'Rasio jarak internal terhadap eksternal.', 'Ambang: Mendekati 0 = Partisi Optimal', '#F1C40F'), use_container_width=True)
    ...
    if 'PCA1' in df.columns and 'PCA2' in df.columns:
        fig_scatter_pca = px.scatter(
            ...
            hover_name='Provinsi',
            text='Provinsi',
            color_discrete_sequence=[palet_warna.get(klaster, warna_default) for klaster in df_pca['Target_Semantic'].unique()],
