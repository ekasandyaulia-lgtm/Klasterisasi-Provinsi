...
if halaman == "Overview":
    ...
    with c1:
            ...
            try:
                ...
                fig_map = px.choropleth(
                    ...
                    color_discrete_map=[palet_warna.get(klaster, warna_default) for klaster in df_map_overview['Target_Semantic'].unique()],
                    hover_name='Provinsi',
                    projection="mercator"
                )
    ...
    with c2:
        ...
        fig = px.bar(df.groupby(['Tahun', 'Target_Semantic']).size().reset_index(name='Jumlah'), 
                     x='Tahun', y='Jumlah', color='Target_Semantic', barmode='stack', color_discrete_map={klaster: palet_warna.get(klaster, warna_default) for klaster in df['Target_Semantic'].unique()})
    ...
    with col_tren:
        ...
        if avail_cols:
            ...
            fig_trend = px.line(df_trend_melted, x='Tahun', y='Nilai Rata-rata', color='Indikator', 
                                markers=True, color_discrete_map={indikator: palet_warna.get(indikator, warna_default) for indikator in df_trend_melted['Indikator'].unique()})
            ...
            for idx, col_name in enumerate(indikator_cols):
                with (grid_c1 if idx % 2 == 0 else grid_c2):
                    fig_small = px.line(
                        ...
                        markers=True,
                        color_discrete_map={klaster: palet_warna.get(klaster, warna_default) for klaster in df_line_filtered['Target_Semantic'].unique()}
                    )
