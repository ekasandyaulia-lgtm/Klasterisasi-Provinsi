...
# Halaman Dinamika Temporal
    ...
    * **Karakteristik Skala RobustScaler**: Penyekalaan berbasis median dan interkuartil (*RobustScaler*) sangat handal dalam mengisolasi bias pencilan (*outliers*), namun tidak mengeliminasi ketimpangan kesenjangan ekonomi riil. Provinsi dengan basis volume ekonomi masif akan selalu menduduki klaster dominan secara persisten.
    ...
    # Tautan Unduh
    def buat_pdf_teks(baris_teks):
        """Membuat berkas PDF sederhana namun valid (struktur PDF asli dengan xref table), tanpa dependency eksternal."""
        konten_teks = "BT /F1 12 Tf 50 750 Td\n"
        for i, baris in enumerate(baris_teks):
            baris_aman = baris.replace("\\", r"\\").replace("(", r"\(").replace(")", r"\)")
            if i == 0:
                konten_teks += f"({baris_aman}) Tj\n"
            else:
                konten_teks += f"0 -18 Td ({baris_aman}) Tj\n"
        konten_teks += "ET"
    ...
    with dl_col2:
        pdf_bytes = buat_pdf_teks([
            "Laporan Kajian Metodologi",
            "Struktur Spasial Klaster Digital",
            "",
            "Dokumen ringkasan metodologi model K-Means (k=4)",
            "untuk segmentasi digital provinsi di Indonesia."
        ])
        st.download_button(
